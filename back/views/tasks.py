...
def process_views_count(end_date, day_window=None, run_code=None, course=None):
    ...
    def create_videos_row(row, vertical_to_associate):
        ...
        """
        video = Video(vertical=vertical_to_associate,
                      block_id=row["id"],
                      duration=row["duration"],
                      watch_time=0)
        return video

    def update_existing_videos(id, previous_objects, queryset):
        previous_objects[id] = queryset.filter(block_id=id).first()
    ...
    def create_views_row(row, video_to_associate):
        """
        Creates ViewOnVideo model instance if it does not already exist
        referencing the video watched by block_id
        """
        view = ViewOnVideo(video=video_to_associate,
                           username=row['username'],
                           coverage=0)
        return view

    def update_existing_views(video_id, user_id, previous_objects, queryset):
        previous_objects[video_id + "-" + user_id] = queryset.filter(
            video__block_id=video_id, username=user_id).first()
    ...
    def make_segment(row, view_to_associate):
        ...
        """
        segment = Segment(view=view_to_associate,
                          time=row['time'],
                          start=int(row['start']),
                          end=int(row['end']))
        return segment
    ...
    def compute_views(dataframe, course_df, date, course_id, code):
        ...
        previous_info = list(
            Video.objects.filter(vertical__course=course_id_df))
        previous_videos = {}
        ...
        if count != 0:
            new_videos = list(
                new_videos_row.apply(lambda row: create_videos_row(
                    row, verticals_to_associate.get(row["id"])),
                                     axis=1))
            Video.objects.bulk_create(new_videos)
            query_videos = Video.objects.filter(vertical__course=course_id_df)
            new_videos_row.apply(lambda row: update_existing_videos(
                row["id"], previous_videos, query_videos),
                                 axis=1)
        ...
        previous_info = list(
            ViewOnVideo.objects.filter(video__vertical__course=course_id_df))
        previous_views = {}
        for b in previous_info:
        ...
        if count != 0:
            new_views = list(
                new_views_row.apply(lambda row: create_views_row(
                    row, previous_videos.get(row["id"])),
                                    axis=1))
            ViewOnVideo.objects.bulk_create(new_views)
            query_views = ViewOnVideo.objects.filter(
                video__vertical__course=course_id_df)
            new_views_row.apply(lambda row: update_existing_views(
                row["id"], row["username"], previous_views, query_views),
                                axis=1)
        ...
        with transaction.atomic():
            ...
            for i in range(0, count, BULK_UPLOAD_SIZE):
                ...
                segments = list(
                    bulk.apply(lambda row: make_segment(
                        row,
                        previous_views.get(row['id'] + "-" + row['username'])),
                               axis=1))
...
@shared_task
def compute_view_batches(initial_date=None,
    ...
    times = pd.date_range(start=initial_date, end=end, freq=time_delta)
    run_code = str(date.today())

    for i in range(1, len(times)):
        logger.info(
            "Processing video views logs for time {} with offset {}".format(
                times[i], time_delta))
