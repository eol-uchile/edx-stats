...
from visits.tasks import compute_completion_batches


class Command(BaseCommand):
    help = 'Compute completion counts from logs on the db from a start date until today. \
    ...
    def handle(self, *args, **options):
        ...
        if options['day_step'] is not None:
            compute_completion_batches(options['start'], time_delta=timedelta(
                days=options['day_step'][0]), final_date=final_date, course=course)
        else:
            compute_completion_batches(
